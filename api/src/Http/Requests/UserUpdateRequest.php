...
namespace Marufsharia\Hyro\Api\Http\Requests;
...
class UserUpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $userId = $this->route('users') ?? $this->user()->id;

        $rules = [
            'name' => ['sometimes', 'string', 'max:255'],
            'email' => ['sometimes', 'string', 'email', 'max:255', "unique:{$table},email,{$userId}"],
        ...
        // Add password policy rules if enabled and password is being updated
        if (Config::get('hyro.security.password_policy.enabled', false) && $this->has('password')) {
            $passwordRules = ['sometimes', 'confirmed'];

<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
    ...
    <form action="{{ route('hyro.admin.settings.update.system') }}" method="POST" class="p-6 space-y-6">
        @csrf
        ...
        <div>
            <label class="flex items-center cursor-pointer">
                <input type="hidden" name="auditing_enabled" value="0">
                <input type="checkbox" name="auditing_enabled" value="1" 
                    {{ $settings['auditing_enabled'] ? 'checked' : '' }}
                    class="sr-only peer">
                <div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
        ...
        <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <form action="{{ route('hyro.admin.settings.cache.config') }}" method="POST">
                    @csrf
                    <button type="submit" 
                ...
                <form action="{{ route('hyro.admin.settings.cache.route') }}" method="POST">
                    @csrf
                    <button type="submit" 
                        class="w-full inline-flex items-center justify-center px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg transition-colors duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form action="{{ route('hyro.admin.settings.cache.app') }}" method="POST">
                    @csrf
                    <button type="submit" 
                        class="w-full inline-flex items-center justify-center px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg transition-colors duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">

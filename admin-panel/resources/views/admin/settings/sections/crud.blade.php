<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
    ...
    <form action="{{ route('hyro.admin.settings.update.crud') }}" method="POST" class="p-6 space-y-6">
        @csrf

        <!-- Generate API -->
        <div>
            <label class="flex items-center cursor-pointer">
                <input type="hidden" name="generate_api" value="0">
                <input type="checkbox" name="generate_api" value="1" 
                    {{ $settings['generate_api'] ? 'checked' : '' }}
                    class="sr-only peer">
        ...
        <div>
            <label class="flex items-center cursor-pointer">
                ...
                <input type="checkbox" name="soft_delete" value="1" 
                    {{ $settings['soft_delete'] ? 'checked' : '' }}
                    class="sr-only peer">
                <div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
        ...
        <div>
            <label class="flex items-center cursor-pointer">
                <input type="hidden" name="auto_permission" value="0">
                <input type="checkbox" name="auto_permission" value="1" 
                    {{ $settings['auto_permission'] ? 'checked' : '' }}
                    class="sr-only peer">
                <div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>

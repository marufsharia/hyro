...
use Marufsharia\Hyro\Plugin\Support\PluginManager as PluginService;
...
{
    use WithFileUploads, WithPagination, WithAlerts;
    ...
    {
        return redirect()->route('hyro.admin.plugins.show', ['pluginId' => $pluginId]);
    }
    ...
    public function activate($pluginId)
    {
        try {
            ...
            // Clear cache so plugin appears immediately
            cache()->forget('hyro.sidebar.items');
            
            $this->logActivity('activate', $pluginId);
            $this->alertSuccess('Success', 'Plugin activated successfully');
            $this->dispatch('refreshPlugins');
            ...
            $this->alertError('Error', 'Activation failed: ' . $e->getMessage());
        }
    }
    ...
    public function deactivate($pluginId)
    {
        try {
            ...
            // Clear cache so plugin disappears immediately
            cache()->forget('hyro.sidebar.items');
            
            $this->logActivity('deactivate', $pluginId);
            $this->alertSuccess('Success', 'Plugin deactivated successfully');
            $this->dispatch('refreshPlugins');
            ...
            $this->alertError('Error', 'Deactivation failed: ' . $e->getMessage());
        }
    }
    ...
    protected function getPluginSidebarEntries($pluginId)
    {
        ...
        // Try to get sidebar entries from SidebarRegistry
        try {
